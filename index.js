...
app.post('/loadGame', (req, res) => {
	GameModel.findOne({'_id': req.body.selectedGame}, function (err, gameObj) {
		if (!err && gameObj) {
			...
			res.render('game.ejs', {"players": game.players, "map": game.map, "turn": game.turn});
		} else {
			next(err);
		}
	})
...
app.get('/turn', (req, res) => {
	// take turn
	const landingX = Math.random();
	const landingY = Math.random();
	game.rules.forEach(function (rule) {
